...
func _process(delta):
	...
	var direction = Vector3(
		...
		float(Input.is_physical_key_pressed(KEY_O)) - float(Input.is_physical_key_pressed(KEY_U)),
		float(Input.is_physical_key_pressed(KEY_K)) - float(Input.is_physical_key_pressed(KEY_I))
	).normalized()
